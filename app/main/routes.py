...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('maim.edit_profile'))
...
@main_blueprint.route('/get_quote', methods=['GET'])
@login_required
def get_quote():
...
@main_blueprint.route('/user/<username>/popup')
@login_required
...
def notifications():
    ...
    return jsonify([{
        'name': notific.name,
        'data': notific.get_data(),
        'timestamp': notific.timestamp
